...
def estimate_average_degree(S, weights):
    return 1.*len(S)/sum(1./weights(v) for v in S)

def estimate_average_degree_ShiftedThinning(S, weights, th=10):
    total=0.
    counter=0.
    S=list(set(S))
    for shift in xrange(th):  
        if len(S[shift::th])>0:
            total   += estimate_average_degree(S[shift::th], weights=weights)
            counter +=1
    return total/counter if counter>0 else -1
